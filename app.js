...
// Normalize older saved bag formats so every club has a usable total distance.
state.profiles.forEach(p=>{
  p.clubs=(p.clubs||[]).map(c=>({
    ...c,
    carry:Number(c.carry ?? c.min ?? c.minimumCarry) || null,
    total:Number(c.total ?? c.max ?? c.maxDistance ?? c.distance) || 0
  })).filter(c=>c.total>0);
});
...
els.adjustments.addEventListener("click",e=>{
  const b=e.target.closest("button");if(!b)return;
  state.shot.adjustment=Number(b.dataset.adjustment);
  [...els.adjustments.children].forEach(x=>x.classList.toggle("selected",x===b));
  save();
});
